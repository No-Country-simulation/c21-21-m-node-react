...
const FAQ = () => {
    ...
    const generalFaqs = [
        ...
        { question: '¿Cómo puedo contactar al soporte en caso de tener un problema?', answer: 'Puedes contactarnos a través de nuestro formulario de contacto en la página, o enviarnos un correo a [email de soporte]. Nuestro equipo te responderá lo antes posible.' },
    ];
    ...
    return (
        <section className="bg-customGray bg-opacity-60 p-12 rounded-lg shadow-lg" id="FAQ">
            <div className="container mx-auto px-4">
                <div className="text-center mb-10">
                    <h2 className="text-3xl font-extrabold text-customWhite sm:text-4xl inline-block border-b-4 border-customGreen pb-2">Preguntas frecuentes</h2>
                </div>
                <div className="bg-customWhite p-8 rounded-lg mb-8">
                    <h3 className="text-2xl font-bold text-customGreen mb-4 inline-block border-b-4 border-customGreen pb-2">Generales</h3>
                    {generalFaqs.map((faq, index) => (
                        <div key={index} className="mb-4">
                            <button
                                className="flex items-center justify-between w-full font-semibold text-customGreen hover:text-customGreen"
                            ...
                            {activeIndex === index && <p className="pl-4 mt-2 text-customBlack">{faq.answer}</p>}
                        </div>
                    ))}
                </div>

                <div className="bg-customWhite p-8 rounded-lg mb-8">
                    <h3 className="text-2xl font-bold text-customGreen mb-4 inline-block border-b-4 border-customGreen pb-2">Startups</h3>
                    {startupFaqs.map((faq, index) => (
                        <div key={index} className="mb-4">
                            ...
                            >
                                {faq.question}
                                <span>
                                    {activeIndex === index + generalFaqs.length ? <FaChevronUp /> : <FaChevronDown />}
                                </span>
                            </button>
                            {activeIndex === index + generalFaqs.length && <p className="pl-4 mt-2 text-customBlack">{faq.answer}</p>}
                        </div>
                    ))}
                </div>

                <div className="bg-customWhite p-8 rounded-lg">
                    <h3 className="text-2xl font-bold text-customGreen mb-4 inline-block border-b-4 border-customGreen pb-2">Inversores</h3>
                    {investorFaqs.map((faq, index) => (
                        <div key={index} className="mb-4">
                            ...
                            >
                                {faq.question}
                                <span>
                                    {activeIndex === index + generalFaqs.length + startupFaqs.length ? <FaChevronUp /> : <FaChevronDown />}
                                </span>
                            </button>
                            {activeIndex === index + generalFaqs.length + startupFaqs.length && <p className="pl-4 mt-2 text-customBlack">{faq.answer}</p>}
                        </div>
                    ))}
                </div>
            </div>
